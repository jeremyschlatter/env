...
fn env(shell: &'static str) -> Result<Vec<(&'static str, String)>> {
    Ok(vec![
        ("NIX_PROFILE", "$HOME/.nix-profile".to_string()),
        ...
        ("LESSCHARSET", "UTF-8".to_string()),

        ("XDG_DATA_DIRS", "$NIX_PROFILE/share".to_string()),

        // https://stackoverflow.com/a/37578829
        ("XDG_DATA_DIRS", "$XDG_DATA_DIRS:/usr/share/ubuntu:/usr/share/gnome:/usr/local/share/:/usr/share/".to_string()),

        ("PATH", "$PATH:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin".to_string()),
        ("PATH", "$HOME/.npm-global/bin:$PATH".to_string()),
        ("PATH", "$NIX_PROFILE/bin:$PATH".to_string()),
        ("PATH", "$HOME/go/bin:$PATH".to_string()),

        // Don't log starship warnings.
        // (Most common warning: git command timed out in large git directory).
        ("STARSHIP_LOG", "error".to_string()),
